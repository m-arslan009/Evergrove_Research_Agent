...
- Plan for exactly {session_minutes} minutes. Do not rescope to a longer or a shorter
  session.
- `run_id`, `generated_at`, `model_used`, `original_task`, `session_duration_minutes`,
  `hops_used` and `sources_examined` are bookkeeping. The system overwrites whatever you
  put there, so any valid placeholder will do — do not spend effort on them.
